...
import { useActiveExchange } from '@/composables/use-active-exchange'

const { isDarkMode, toggleDarkMode } = useLayout()
const { exchanges, fetchExchanges } = useExchanges()
const { activeExchange, setSelectedExchange } = useActiveExchange()
    ...
    <div
        ...
        <div class="flex justify-between items-center">
            <div class="flex gap-3 items-center">
                <svg width="35" height="40" viewBox="0 0 35 40" fill="none" class="w-8"
                ...
                <span class="hidden sm:flex flex-col"><span
                        class="text-xl font-light text-surface-700 dark:text-surface-100 leading-none">PrimeVue
                        Examples</span>
                    <span class="text-sm font-medium text-primary leading-tight">Volt + Nuxt Unstyled + Tailwindcss
                        v4</span></span>
            </div>
            <div class="flex items-center gap-2">
                <Select v-model="activeExchange" :options="exchanges" optionLabel="name" placeholder="Select a Exchange"
                    class="w-full md:w-56" @update:modelValue="setSelectedExchange" />
                <button type="button"
